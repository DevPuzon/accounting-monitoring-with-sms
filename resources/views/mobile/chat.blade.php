...
<script>
    function chatRender(){
        ...
        let admin_user_id = '262'; 
        fetch( `${ '{{url('message/get-user')}}' }?user_id=${admin_user_id}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            var data = JSON.parse(result).data; 
            updateConvoItem(data.name,data.pic_path,data.id);
        })
